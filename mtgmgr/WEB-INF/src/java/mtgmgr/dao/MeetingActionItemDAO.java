 ...
 * Filename: MeetingActionItemDAO.java
...
/**
 * <p>ClassName:	 <code>MeetingActionItemDAO</code></p>
...
public class MeetingActionItemDAO extends DAOFactory {
	...
	public List<MeetingActionItemDTO> findAllByMtgRmId(int mtgRmId) {
		...
		List<MeetingActionItemDTO> results = findAllBy(criteria);
		// post processing - attach all assignees as comma-delimited string
		attachAssignees(results);
		// post processing - attach all associated notes to each action item in list
		for (MeetingActionItemDTO actionItem : results)
			actionItem.setNotes(findOneByIdNotes(actionItem.getActionItemId()));
