(function(jQuery){jQuery.fn.extend({elastic:function(){var mimics=['paddingTop','paddingRight','paddingBottom','paddingLeft','fontSize','lineHeight','fontFamily','width','fontWeight'];return this.each(function(){if(this.type!='textarea'){return false}var $textarea=jQuery(this),$twin=jQuery('<div />').css({'position':'absolute','display':'none','word-wrap':'break-word'}),lineHeight=parseInt($textarea.css('line-height'),10)||parseInt($textarea.css('font-size'),'10'),minheight=parseInt($textarea.css('height'),10)||lineHeight*3,maxheight=parseInt($textarea.css('max-height'),10)||Number.MAX_VALUE,goalheight=0,i=0;if(maxheight<0){maxheight=Number.MAX_VALUE}$twin.appendTo($textarea.parent());var i=mimics.length;while(i--){$twin.css(mimics[i].toString(),$textarea.css(mimics[i].toString()))}
/* modified by ME */
function setHeightAndOverflow(height,overflow) {
	var heightToSet   = Math.floor(parseInt(height,10)),
		currentHeight = $textarea.height(),
		maxHeight     = $textarea.attr('maxheight');
	if(currentHeight!=heightToSet && heightToSet<maxHeight)
	{
		$textarea.css({'height':heightToSet+'px','overflow':overflow});
	}
	else if (currentHeight>maxHeight || heightToSet>=maxHeight)
	{
		$textarea.css({'height':maxHeight+'px','overflow':'visible'});
	}
}
/* end modification */
